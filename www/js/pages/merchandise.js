const merchandise = {
  async render() {
    ...
    <div class="card">
      ...
      <div class="stat-card revenue" style="margin-bottom:16px">
        <h3>Total Spent</h3><div class="value">${App.fmtMoney(total)}</div>
      </div>
      ...
      <form onsubmit="merchandise.add(event)" style="background:var(--surface-alt);border-radius:8px;padding:14px;margin-bottom:20px;border:1px solid var(--border)">
        <h3 style="font-size:15px;margin-bottom:12px">+ Add Purchase</h3>
        <div class="form-group">
          ...
          </select>` : '<p style="color:#e74c3c;font-size:14px">Add a store first below.</p>'}
        </div>
        <div class="form-group">
          <label>Date</label>
        ...
        <div class="form-group">
          ...
          <input type="text" id="merch-notes" placeholder="Any notes…">
        </div>
        ${vendors.length ? `<button type="submit" class="btn btn-success">Save Purchase</button>` : ''}
      </form>
    </div>
    ...
    ${Object.keys(grouped).length ? Object.entries(grouped).map(([vName, items]) => {
      ...
      return `<div class="card">
        ...
        <table><thead><tr><th>Date</th><th>Amount</th><th>Receipt</th><th>Notes</th><th></th></tr></thead><tbody>
          ${items.map(p=>`<tr>
    ...
    <div class="card">
      ...
      <form onsubmit="merchandise.addVendor(event)" style="display:flex;gap:10px;margin-bottom:16px">
        <input type="text" id="vendor-name" placeholder="Store name" style="flex:1" required>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        ...
        </span>`).join('') || '<p style="color:var(--text-muted)">No stores yet.</p>'}
      </div>
    </div>
  ...
  async add(e) {
    ...
    const fileInput = document.getElementById('merch-receipt');

const orders = {
  async render() {
    ...
    <div class="card">
      ...
      <form onsubmit="orders.create(event)" style="background:var(--surface-alt);border-radius:8px;padding:14px;margin-bottom:20px;border:1px solid var(--border)">
        <h3 style="font-size:15px;margin-bottom:12px">+ New Order List</h3>
        <div class="form-group">
          <label>List Name / Department</label>
          <input type="text" id="order-name" placeholder="e.g. Deli, Bakery, Produce" required>
        </div>
        ...
        return `<div class="card" style="margin-bottom:12px;border:1px solid var(--border)">
          <div style="display:flex;justify-content:space-between;align-items:flex-start">
            <div>
              ...
              <p style="font-size:13px;color:var(--text-muted);margin-top:4px">${pending} of ${total} items pending</p>
            </div>
            <button class="btn btn-danger btn-sm" onclick="orders.del(${list.id})">✕</button>
          </div>
          <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
            <button class="btn btn-primary btn-sm" onclick="orders.open(${list.id})">View List</button>
            <button class="btn btn-sm" style="background:#8e44ad" onclick="orders.share(${list.id})">📤 Share Link</button>
          </div>
        </div>`;
      }).join('') : '<p style="color:var(--text-muted);text-align:center;padding:20px 0">No order lists yet.</p>'}
    </div>`;
  },
  ...
  async open(id) {
    ...
    container.innerHTML = orders._detailHtml(list);
  },
  ...
  _detailHtml(list) {
    ...
    <div class="card">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
        <button class="btn btn-sm" onclick="App.nav('orders')">← Back</button>
      ...
      <form onsubmit="orders.addItem(event,${list.id})" style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap">
        <input type="text" id="item-name" placeholder="Item name" style="flex:1;min-width:160px" required>
        <input type="number" id="item-qty" value="1" min="1" style="width:80px">
        <button type="submit" class="btn btn-success">Add</button>
      </form>

      ${pending.length ? `<h3 style="font-size:15px;color:var(--heading);margin-bottom:10px">Pending (${pending.length})</h3>
      ...
      ${done.length ? `<h3 style="font-size:15px;color:var(--text-muted);margin-top:20px;margin-bottom:10px">Done (${done.length})</h3>
      ...
      ${!items.length ? '<p style="color:var(--text-muted);text-align:center;padding:30px 0">List is empty. Add items above.</p>' : ''}
    </div>`;
  },
  ...
  async share(id) {
    const list = await DB.get('orders', id);
    if (!list) return;
    // Build a simple shareable text summary
    const items = (list.items || []).filter(i => !i.checked);
    const text = `📋 ${list.name} Order List\n\n` +
      (items.length ? items.map(i => `• ${i.name}${i.qty > 1 ? ` ×${i.qty}` : ''}`).join('\n') : 'No pending items') +
      `\n\nView on StoreLedger app`;

    if (navigator.share) {
      navigator.share({ title: `${list.name} Order List`, text }).catch(() => {});
    } else {
